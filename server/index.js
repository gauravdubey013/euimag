  ...
  async (req, res) => {
    try {
      ...
      if (profileimage && profileimage.originalname) {
        ...
        const pfpPathPublic = `../client/public/users/profileImages/${
          username + "_" + profileimage.originalname
        }`;
        await writeFile(pfpPathPublic, bufferPfp);

        pfpPath = `/users/profileImages/${
          username + "_" + profileimage.originalname
        }`;
      } else {
        pfpPath = "NaN";
      }
      // updating pfp file path in User collection
...
//     return res
//       .status(400)
//       .send({ status: "error", message: "Please upload an image." });
//   }

//   const imageName = req.file.filename;
//   try {
//     const image = await ImageDetails.create({ image: imageName });
//     res.json({ status: "ok", image });
//   } catch (error) {
//     console.error("Database error:", error);
//     res
//       .status(500)
//       .json({ status: "error", message: "Error saving image in database" });
//   }
// });
